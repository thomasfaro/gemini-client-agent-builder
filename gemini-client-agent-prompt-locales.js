...
3. Query using the strategies below (parallelize when possible):`,
      ...
      crossSourceRules: [
        ...
        "**Traceability — inline clickable links:** render every source as a **markdown hyperlink** `[label](real-url)` using the actual URL the connector returned — never a bare tag. Patterns: `[Zendesk #116854](https://<subdomain>.zendesk.com/agent/tickets/116854)`, `[ADVSEG-9857](https://<org>.atlassian.net/browse/ADVSEG-9857)`, `[Meet notes — 12 Mar](drive-file-url)`, `[email — renewal thread](gmail-permalink)`, `[event — QBR](calendar-event-url)`, `[Salesforce — Account](record-url)`. **Zendesk URLs:** always use the agent UI form `https://<subdomain>.zendesk.com/agent/tickets/<id>` — if the connector returned an API URL (`.../api/v2/tickets/<id>` or `.json`), rewrite it (keep the numeric ID, drop `/api/v2/` and `.json`). Fall back to a plain bracket tag (e.g. `[Slack]`) **only** when the connector exposes no URL/ID. Every Jira issue, Zendesk ticket, Drive/Meet file, email, and meeting cited must carry its clickable link inline.",
      ...
      sopB_body:
        'For specific questions (e.g. "open tickets", "Jira escalations", "last client meeting", "Gemini meeting notes"): query only **relevant** connectors, **infer the time window from the user\'s wording** (dates, "last week", "open", "recent", etc.), cite sources, and **answer inline directly** with a **compact source index** listing every connector you queried (with status). Hand off to the report writer only if the user explicitly asks for a formatted brief.',
      sopAlertFastPath:
        '### B+. Alert fast-path\nWhen the user asks **only** about risks/alerts/escalations (e.g. "any risks?", "anything urgent?", "are we on fire?"): skip the full connector sweep. Query only **Zendesk** (urgent/high/escalated/SLA-at-risk), **Jira** (PF + escalation-class), **Gmail** (client emails unanswered > 24 business hours), and **Slack** (churn/negative-sentiment signals). **Return the 🚨 Alerts section directly** plus a compact source index of just those connectors — no report writer hand-off.',
...
3. Abfrage gemäß den Strategien unten (parallelisieren wenn möglich):`,
      ...
      crossSourceRules: [
        ...
        "**Nachvollziehbarkeit — klickbare Inline-Links:** Stelle jede Quelle als **Markdown-Link** `[Label](echte-url)` dar, mit der tatsächlich vom Connector gelieferten URL — niemals nur ein Tag. Muster: `[Zendesk #116854](https://<subdomain>.zendesk.com/agent/tickets/116854)`, `[ADVSEG-9857](https://<org>.atlassian.net/browse/ADVSEG-9857)`, `[Meet-Notizen — 12. März](drive-datei-url)`, `[E-Mail — Renewal-Thread](gmail-permalink)`, `[Termin — QBR](calendar-event-url)`, `[Salesforce — Account](datensatz-url)`. **Zendesk-URLs:** verwende immer die Agent-UI-Form `https://<subdomain>.zendesk.com/agent/tickets/<id>` — wenn der Connector eine API-URL geliefert hat (`.../api/v2/tickets/<id>` oder `.json`), schreibe sie um (numerische ID behalten, `/api/v2/` und `.json` entfernen). Greife **nur** dann auf ein einfaches Tag (z. B. `[Slack]`) zurück, wenn der Connector keine URL/ID liefert. Jedes zitierte Jira-Issue, Zendesk-Ticket, jede Drive-/Meet-Datei, E-Mail und jedes Meeting muss seinen klickbaren Link inline tragen.",
      ...
      sopB_body:
        'Bei gezielten Fragen (z. B. « offene Tickets », « Jira-Eskalationen », « letztes Kundenmeeting », « Gemini-Meeting-Notizen »): nur **relevante** Connectors abfragen, **Zeitfenster aus der Formulierung ableiten** (Daten, « letzte Woche », « offen », « kürzlich », etc.), Quellen zitieren und **direkt inline antworten** mit einem **kompakten Quellenindex** jedes abgefragten Connectors (mit Status). Nur an den Report-Writer übergeben, wenn der Nutzer ausdrücklich einen formatierten Brief verlangt.',
      sopAlertFastPath:
        '### B+. Alert-Schnellpfad\nWenn der Nutzer **ausschließlich** nach Risiken/Alerts/Eskalationen fragt (z. B. « irgendwelche Risiken? », « etwas Dringendes? »): überspringe den vollständigen Sweep. Frage nur **Zendesk** (urgent/hoch/eskaliert/SLA-gefährdet), **Jira** (PF + Eskalations-Klasse), **Gmail** (Kunden-E-Mails unbeantwortet > 24 Geschäftsstunden) und **Slack** (Churn-/negative-Sentiment-Signale) ab. **Gib die 🚨 Alerts-Sektion direkt zurück** plus einen kompakten Quellenindex nur dieser Connectoren — ohne Übergabe an den Report-Writer.',
